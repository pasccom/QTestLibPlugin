...
public:
     ...
     */
    inline Utils::Id id() const override {return Utils::Id(Constants::XmlQTestLibParserFactoryId).withSuffix(mBase.id().toString());}
    /*!
     * \brief Base factory
     *
...
template<class B>
AbstractTestParser* XMLQTestLibParserFactory<B>::getParserInstance(ProjectExplorer::RunControl* runControl) const
{
    ...
    return new XMLQTestLibParser(runControl);
}
