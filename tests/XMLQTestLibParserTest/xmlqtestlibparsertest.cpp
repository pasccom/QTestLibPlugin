...
class XMLQTestLibParserTest : public QObject
{
    Q_OBJECT
public:
    inline XMLQTestLibParserTest(void) {mRandom = QRandomGenerator::global();}
private Q_SLOTS:
...
void XMLQTestLibParserTest::runTest(const QString& qtVersion, const QString& testName, QTestLibModelTester::Verbosity verbosity)
{
    ...
    checkTest(model, results, qtVersion, testName, qMax(QTestLibModelTester::Normal, verbosity)); // NOTE When running in XML silent is equal to normal

    delete model;
    delete parser;
}

void XMLQTestLibParserTest::runMakeCheck(const QString& qtVersion, const QString& testName, QTestLibModelTester::Verbosity verbosity)
{
    // Creation of Runnable
    Utils::ProcessRunData runnable;
    runnable.workingDirectory = Utils::FilePath::fromString(TESTS_DIR "/" + qtVersion + "/" + testName + "/");
...
void XMLQTestLibParserTest::checkTest(const QAbstractItemModel *model, QLinkedList<QTestLibPlugin::Internal::TestModelFactory::ParseResult> results, const QString& qtVersion, const QString& testName, QTestLibModelTester::Verbosity verbosity)
{
...
QLinkedList<QTestLibPlugin::Internal::TestModelFactory::ParseResult> XMLQTestLibParserTest::executeTest(QTestLibPlugin::Internal::AbstractTestParser* parser, const Utils::ProcessRunData& runnable, const QLinkedList<EnvironmentVariable>& addToEnv)
{
